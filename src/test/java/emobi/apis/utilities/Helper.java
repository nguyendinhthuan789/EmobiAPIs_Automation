...
import emobi.apis.config.ConfigSettings;
import emobi.apis.config.PropertySettingStoreUtil;
...
public class Helper {
    ...
    public static void createEnvironmentProperties(String filePath) {
        ...
        if (Helper.getDomainBaseOnEnv().equals(config.getDevEnvParnerDomain())) {
            prop.setProperty("Domain:", Helper.getDomainBaseOnEnv());
            prop.setProperty("Enviroment:", "dev");
        }
        try {
            PropertySettingStoreUtil.saveSettings(prop, filePath, "");
            log.info(
                    "Class Helper | Method createEnvironmentProperties | saveSettings : " + prop + " | "
                            + filePath);
        } catch (IOException e) {
            log.error(
                    "Class Helper | Method createEnvironmentProperties | Exception desc : " + e.getMessage());
        }
    }
    ...
    public static String setDomain(String domain) {
        String context;
        switch (domain) {
            case ConfigSettings.DEV_ENV_PARNER_DOMAIN:
                context = System.getProperty(ConfigSettings.DEV_ENV_PARNER_DOMAIN, config.getDevEnvParnerDomain());
                break;
            case ConfigSettings.DEV_ENV_LPAS_DOMAIN:
                context = System.getProperty(ConfigSettings.DEV_ENV_LPAS_DOMAIN, config.getDevEnvLpasDomain());
                break;
            default:
                context = System.getProperty(ConfigSettings.ENV_DEFAULT, config.getEnvDefault());
        }
        return context;
    }

    public static String getDomainBaseOnEnv() {
        return setDomain(System.getProperty("testEnv"));
    }
}
