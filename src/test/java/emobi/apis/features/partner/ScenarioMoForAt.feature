...
Feature: Tracking request flow type MSISDN + MO Functionality
  ...
  Scenario: User GET tracking request flow MSISDN + MO of AT country successful with null msisdn
    ...
    Then Response successful status code is 400 after tracking request of flow msisdn mo with null msisdn
    Then Response successful message after tracking request of flow msisdn mo with null msisdn
      | message                  | status |
      | Invalid mandatory fields | false  |

  Scenario: User GET tracking request flow MSISDN + MO of AT country successful with invalid campaign
    Given User call the API tracking request of flow msisdn mo with with invalid campaign
      | campaignID      | tpClickId | msisdn | userAgent                                                                                                                                                         | ip            | portalDomain                              |
      | invalidCampaign |           |        | Mozilla%2F5.0%20(Macin tosh%3B%20Intel%20Ma c%20OS%20X%2010_15_7)%20AppleWebKit%2F 537.36%20(KHTML%2C%20like%20Gecko)%20C hrome%2F91.0.4472.101%20Safari%2F537.36 | 129.20.20.100 | https://moviplus_sample_landing_page.html |
    Then Response successful status code is 400 after tracking request of flow msisdn mo with invalid campaign
