...
public class C2sTracking {
    ...
    public void response_successful_message_status_keyword_shortcode_and_trackingcode_not_null_after_tracking_request_of_adpeako_flow_c2s_ba_eronet(List<List<String>> list) {
        ...
        log.info("message is: " + c2sController.getJsonPathHasKey("message"));
        Assert.assertEquals(result.get(4), c2sController.getJsonPathHasKey("message"));
        log.info("status is: " + c2sController.getJsonPathHasKey("status"));
        Assert.assertEquals(result.get(5), c2sController.getJsonPathHasKey("status"));
        log.info("keyword is: " + c2sController.getJsonPathHasKey("data.keyword"));
        Assert.assertEquals(result.get(6), c2sController.getJsonPathHasKey("data.keyword"));
        log.info("shortcode is: " + c2sController.getJsonPathHasKey("data.shortcode"));
        Assert.assertEquals(result.get(7), c2sController.getJsonPathHasKey("data.shortcode"));
        log.info("tracking_code is: " + c2sController.getJsonPathHasKey("data.tracking_code"));
        Assert.assertNotNull(c2sController.getJsonPathHasKey("data.tracking_code"));
    }
